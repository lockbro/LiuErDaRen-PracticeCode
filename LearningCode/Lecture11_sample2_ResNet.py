...
class ResNetNormalBlock(nn.Module):
    def __init__(self, channels):
        super(ResNetNormalBlock, self).__init__()
    ...
    def forward(self, x):
        y = F.relu(self.conv1(x))  # 卷积层+relu激活
        y = self.conv2(y)
        return F.relu(y + x)  # 残差连接+relu激活
...
class Model(nn.Module):
    def __init__(self):
        ...
        # resnet模块
        self.resblock1 = ResNetNormalBlock(16)
        self.resblock2 = ResNetNormalBlock(32)
